  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      if (response.Code == 200) {
        toaster.success("Assistant deleted successfully!");
        refetch();
      } else {
        toaster.error("Failed to delete assistant!");
      }
    } catch (error) {
      toaster.error("An error occurred while deleting the assistant.");
    }
  };
  ...
  return (
    <>
      <div className="space-y-6">
        <ComponentCard title="AI Assistants">
          <>
            ...
            <DataGrid
              ...
              isLoading={isLoading}
              data={assistants || []}
              text={`Total Assistants (${rowTotal})`}
              currentPage={offset}
              totalPage={rowTotal}
              isLine={true}
