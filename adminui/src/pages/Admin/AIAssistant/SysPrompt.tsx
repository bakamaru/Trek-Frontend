...
export default function SysPrompt() {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      if (response === true) {
        toaster.success("System Prompt deleted successfully!");
        refetch();
      } else {
        toaster.error("Failed to delete System Prompt!");
      }
    } catch (error) {
      toaster.error("An error occurred while deleting the System Prompt.");
    }
  };
  ...
  return (
    <>
      <div className="space-y-6">
        <ComponentCard title="System Prompts">
          <>
            ...
            <DataGrid
              ...
              isLoading={isLoading}
              data={systemPrompts || []}
              text={`Total System Prompts (${rowTotal})`}
              currentPage={offset}
              totalPage={rowTotal}
              isLine={true}
