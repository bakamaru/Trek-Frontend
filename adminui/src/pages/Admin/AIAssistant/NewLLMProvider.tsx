...
const NewLLMProvider = () => {
  ...
  useEffect(() => {
    // Check if the URL contains "edit" to determine if it's edit mode
    if (location.pathname.includes("edit") && id) {
      setIsEditMode(true);
      setProviderId(parseInt(id, 10)); // Set providerId from query parameter
    } else {
      setIsEditMode(false);
      setProviderId(0);
    }
  ...
  const handleDefaultChange = (clickedIndex: any) => {
    // Get all the current values from the field array
    const allModels = getValues("LLMMs");

    // Loop through all the models and set their `UseDefault` to false,
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="grid grid-cols-3 gap-4">
          ...
          <ComponentCard className="col-span-2" title={`Config`}>
            <div className="grid grid-flow-row auto-rows-max">
              ...
              {configFields.map((item, index) => (
                <div key={item.id} className="flex items-end gap-2 mb-2">
                  ...
                  <InputField
                    type="text"
                    labelName="Setting Key"
                    placeholder="Key"
                    {...register(`Configs.${index}.SettingKey`, { required: "Setting key is required." })}
                    error={!!errors.Configs?.[index]?.SettingKey}
                    errorMsg={errors.Configs?.[index]?.SettingKey?.message}
                  />
                  <InputField
                    type="text"
                    labelName="Setting Value"
                    placeholder="Value"
                    {...register(`Configs.${index}.SettingValue`, { required: "Setting Value is required." })}
                ...
                <Checkbox
                  ...
                  {...register(`LLMMs.${index}.UseDefault`, {
                    ...
                    validate: () => {
                      ...
                      return isAtLeastOneChecked || "At least one model must be set as default.";
                    }
                  })}
                  onChange={(e) => {
                    if (e.target.checked) {
