            ...
            <div className="p-6 sm:p-8">
                <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                        {editMenuId ? "Edit Menu Item" : "Add New Menu Item"}
                    </h3>
                </div>
                ...
                {!isLoadingDetails && (
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-1.5">
                                <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Icon Class</label>
                                <input
                                    {...register("icon")}
                                    className="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:text-white dark:placeholder-gray-500"
                            ...
                            <div className="space-y-1.5">
                                <label className="text-sm font-medium text-gray-700 dark:text-gray-300">CSS Class</label>
                                <input
                                    {...register("cssClass")}
                                    className="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:text-white dark:placeholder-gray-500"
                        ...
                        <div className="flex flex-wrap gap-6 p-4 rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-gray-800">
                            ...
                            <label className="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span className="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">System Menu</span>
                            </label>
                        </div>

                        <div className="space-y-3">
                            <h4 className="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-wider">Role Permissions</h4>
                                        ...
                                        <div key={role.id} className="flex items-center justify-between p-3 hover:bg-gray-50 dark:hover:bg-white/[0.03] rounded-lg transition-colors">
                                            ...
                                            <div className="flex items-center">
                                                <label className={`flex items-center gap-2 ${isSuperAdmin ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'}`}>
                                                    ...
                                                    <span className="text-xs font-medium text-gray-600 dark:text-gray-400">Allow Access</span>
                                                </label>
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} className="max-w-md">
            <div className="p-6 sm:p-8">
                <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                        {initialData?.menuGroupId ? "Edit Menu Group" : "Add Menu Group"}
                    </h3>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-6 p-4 rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-gray-800">
                        ...
                        <label className="flex items-center gap-3 cursor-pointer group">
                            ...
                            <span className="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">System Group</span>
                        </label>
                    </div>
...
const MenuManagement = () => {
    ...
    const handleSaveOrder = async (updatedItems: FlatMenuItem[]) => {
        ...
        try {
            ...
            toaster.success("Menu order updated");
        } catch (error) {
            toaster.error("Failed to update menu order");
        }
    };
    ...
    const handleSaveMenu = async (data: MenuSaveRequest) => {
        try {
            ...
            toaster.success("Menu saved successfully");
            setIsModalOpen(false);
            refetchMenus();
        } catch (error) {
            toaster.error("Failed to save menu");
        }
    };
    ...
    const handleSaveGroup = async (data: MenuGroup) => {
        try {
            ...
            toaster.success("Menu group saved successfully");
            setIsGroupModalOpen(false);
            refetchGroups();
        } catch (error) {
            toaster.error("Failed to save menu group");
        }
    };
    ...
    const handleDeleteGroup = async (id: number) => {
        if (window.confirm("Are you sure you want to delete this group?")) {
            try {
                ...
                toaster.success("Menu group deleted successfully");
                refetchGroups();
                ...
                toaster.error("Failed to delete menu group");
            }
        }
    };
    ...
    return (
        <>
            <PageMeta
                ...
                description="Manage your frontend and backend menus, permissions and order."
            />
                        ...
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                                    ...
                                    <DragOverlay>
                                        {activeId ? (
                                            <div className="p-3 bg-white border border-brand-500 rounded-xl shadow-theme-lg opacity-90 dark:bg-gray-800 dark:border-brand-400">
                                                <span className="font-medium text-gray-900 dark:text-white">Moving Item...</span>
                                            </div>
                                        ) : null}
                                    </DragOverlay>
                                </DndContext>
                            ...
                            {!isMenusLoading && items.length === 0 && (
                                <div className="flex flex-col items-center justify-center h-40 text-gray-400 dark:text-gray-500">
                                    ...
                                    <p className="text-sm">No menus found in this group.</p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] h-[calc(100vh-120px)] flex items-center justify-center text-center p-10">
                        <div className="max-w-md">
                            ...
                            <p className="text-gray-500 dark:text-gray-400 mb-8">
                                Select a menu group from the left to view its items. You can add, edit, or delete groups and manage their menu items via drag and drop.
                            </p>
                            <div className="grid grid-cols-2 gap-4 text-left">
                                <div className="p-4 rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-gray-800">
                                    <h4 className="font-semibold text-gray-900 dark:text-white mb-1">Groups</h4>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">Create groups to organize menus (e.g., Frontend, Backend).</p>
                                </div>
                                <div className="p-4 rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-gray-800">
                                    <h4 className="font-semibold text-gray-900 dark:text-white mb-1">Permissions</h4>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">Control visibility for different user roles per menu item.</p>
                                </div>
                            </div>
                        </div>
                    </div>
