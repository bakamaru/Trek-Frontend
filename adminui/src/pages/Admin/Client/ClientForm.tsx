...
const TEMPLATES: ClientTemplate[] = [
    ...
    },
    {
        ...
        description: "React, Angular, Vue, Blazor WASM. Uses PKCE.",
        icon: Layout,
        defaultValues: {
            clientType: "public",
            consentType: "explicit",
    ...
    },
    {
        ...
        description: "Server-side apps (MVC, Next.js). Uses Client Secret.",
        icon: Globe,
        defaultValues: {
    ...
    },
    {
        ...
        description: "Daemon services, cron jobs. client_credentials flow.",
        icon: Server,
        defaultValues: {
            clientType: "confidential",
    ...
    },
    {
        id: "mobile",
        title: "Mobile / Native",
        description: "iOS, Android. Uses PKCE and custom schemes.",
        icon: Smartphone,
        defaultValues: {
            clientType: "public",
            consentType: "explicit",
    ...
    },
    {
        id: "device",
        title: "Device / IoT",
        description: "Devices with limited input capabilities.",
        icon: Monitor,
        defaultValues: {
            clientType: "public",
            consentType: "explicit",
...
const StepIndicator = ({ currentStep }: { currentStep: number }) => {
    const steps = ["Type", "Basics", "Settings", "Review"];
    return (
            ...
            {steps.map((label, index) => (
                <div key={label} className="flex items-center">
                    <div className={`flex flex-col items-center relative z-10`}>
                        ...
                        <span className={`text-xs mt-2 font-medium ${index <= currentStep ? "text-brand-500" : "text-gray-500"}`}>
                            {label}
                        </span>
                    </div>
        ...
        <div className="max-w-2xl mx-auto space-y-6">
            <div className="space-y-2">
                ...
                <input
                    ...
                    disabled={isEditMode}
                    className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-2 focus:ring-brand-500/50 outline-none disabled:opacity-60 disabled:cursor-not-allowed"
                    placeholder="e.g. my-awesome-app"
                />
                {isEditMode && <p className="text-xs text-gray-500">Client ID cannot be changed after creation</p>}
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-6">
                <h3 className="text-lg font-semibold border-b pb-2 dark:border-gray-700">Detailed Configuration</h3>
                ...
                {clientType === "confidential" && (
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            type="password"
                            placeholder={isEditMode ? "Leave empty to keep unchanged" : "Leave empty to generate automatically"}
                            className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-2 focus:ring-brand-500/50 outline-none"
                        />
                        <p className="text-xs text-gray-500">Required for confidential clients (Web Apps, APIs, etc).</p>
                    </div>
                )}

                <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Client Type</label>
                    <select
                        {...register("clientType")}
                        className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-2 focus:ring-brand-500/50 outline-none"
                    >
                        <option value="public">Public (SPA, Mobile)</option>
                        <option value="confidential">Confidential (Web App, M2M)</option>
                    </select>
                </div>

                <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Consent Type</label>
                    <select
                        {...register("consentType")}
                        className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-2 focus:ring-brand-500/50 outline-none"
                    >
                        <option value="explicit">Explicit (User must approve)</option>
                        <option value="external">External</option>
                        <option value="implicit">Implicit</option>
                        <option value="systematic">Systematic (Trust implicitly)</option>
                    </select>
                </div>
                    ...
                    <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                            <TagsInput
                                ...
                                placeholder="https://app.com/logout-callback"
                                shouldCreateNew={true}
                                predefinedTags={[]}
                            />
                        )}
            ...
            <div className="space-y-6">
                <h3 className="text-lg font-semibold border-b pb-2 dark:border-gray-700">Permissions & Capabilities</h3>
...
const ReviewStep = ({ getValues }: { getValues: UseFormReturn<ClientFormData>["getValues"] }) => {
    ...
    return (
        <div className="max-w-3xl mx-auto bg-gray-50 dark:bg-gray-800/50 rounded-xl p-8 border border-gray-200 dark:border-gray-700">
            <h3 className="text-xl font-bold mb-6 text-center">Review Client Configuration</h3>

            <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                <div>
                    <dt className="text-sm font-medium text-gray-500">Client ID</dt>
                    <dd className="mt-1 text-base font-semibold">{values.clientId}</dd>
                </div>
                <div>
                    <dt className="text-sm font-medium text-gray-500">Display Name</dt>
                    <dd className="mt-1 text-base font-semibold">{values.displayName}</dd>
                </div>
                <div>
                    <dt className="text-sm font-medium text-gray-500">Type</dt>
                    <dd className="mt-1 text-base font-semibold capitalize">{values.clientType}</dd>
                </div>
                <div>
                    <dt className="text-sm font-medium text-gray-500">Consent</dt>
                    <dd className="mt-1 text-base font-semibold capitalize">{values.consentType}</dd>
                </div>

                <div className="md:col-span-2">
                    <dt className="text-sm font-medium text-gray-500">Redirect URIs</dt>
                ...
                <div className="md:col-span-2">
                    <dt className="text-sm font-medium text-gray-500">Permissions ({values.permissions?.length})</dt>
                    <dd className="mt-1 max-h-40 overflow-y-auto text-sm text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 p-3 rounded border">
    ...
    const onSubmit = async (data: ClientFormData) => {
        try {
            ...
            if (response.Code == 200) {
                toaster.success(`Client ${isEditMode ? "updated" : "created"} successfully!`);
            ...
            console.error(err);
            toaster.error(err?.data?.message || "An error occurred while saving the client.");
        }
    };

    if (isEditMode && isLoadingDetails) return <div className="p-10 text-center">Loading...</div>;
        ...
        <div className="max-w-6xl mx-auto p-6">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold dark:text-white">{isEditMode ? "Edit Client" : "Create New Client"}</h1>
                        <p className="text-gray-500">Step {currentStep + 1} of 4</p>
                    </div>
                </div>
            </div>
