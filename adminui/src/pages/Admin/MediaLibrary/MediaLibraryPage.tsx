...
const MediaLibraryPage: React.FC = () => {
    ...
    const handleSaveNewFolder = async (data: DirectoryViewModel) => {
        try {
            ...
            if (result.Success) {
                toaster.success(result.Message || 'Folder created successfully');
                setNewFolderModalOpen(false);
                refetch();
            } else {
                toaster.error(result.Message || 'Failed to create folder');
            }
        } catch (error: any) {
            const errorMessage = error?.data?.Message || error?.message || 'An error occurred while creating folder';
    ...
    const handleFileUpload = async (file: File, dir: string) => {
        try {
            ...
            if (result.Success) {
                toaster.success(result.Message || 'File uploaded successfully');
                setUploadModalOpen(false);
                refetch();
            } else {
                toaster.error(result.Message || 'Failed to upload file');
            }
        } catch (error: any) {
            // Extract error message from RTK Query error
            const errorMessage = error?.data?.Message || error?.message || 'An error occurred while uploading file';
            toaster.error(errorMessage);
    ...
    const handleCopy = () => {
        if (selectedItems.length === 0) {
            toaster.error('Please select items to copy');
            return;
        }
    ...
    const handleCut = () => {
        if (selectedItems.length === 0) {
            toaster.error('Please select items to cut');
            return;
        }
    ...
    const handleCopyMoveConfirm = async (destinationDir: string) => {
        try {
            if (operationMode === 'copy') {
                ...
                if (result.Success) {
                    toaster.success(result.Message || 'Files copied successfully');
                    setCopyMoveModalOpen(false);
                    setSelectedItems([]);
                    refetch();
                } else {
                    toaster.error(result.Message || 'Failed to copy files');
                }
                ...
                if (result.Success) {
                    toaster.success(result.Message || 'Files moved successfully');
                    setCopyMoveModalOpen(false);
                    setSelectedItems([]);
                    refetch();
                } else {
                    toaster.error(result.Message || 'Failed to move files');
                }
            }
        } catch (error: any) {
            const errorMessage = error?.data?.Message || error?.message || 'An error occurred during the operation';
            toaster.error(errorMessage);
    ...
    const handleDelete = () => {
        if (selectedItems.length === 0) {
            toaster.error('Please select items to delete');
            return;
        }
    ...
    const performDelete = async () => {
        try {
            ...
            if (result.Success) {
                toaster.success(result.Message || 'Items deleted successfully');
                setSelectedItems([]);
                refetch();
            } else {
                toaster.error(result.Message || 'Failed to delete items');
            }
        } catch (error: any) {
            const errorMessage = error?.data?.Message || error?.message || 'An error occurred while deleting items';
            toaster.error(errorMessage);
    ...
    const handleSaveRename = async (oldName: string, newName: string, dir: string) => {
        try {
            ...
            if (result.Success) {
                toaster.success(result.Message || 'Item renamed successfully');
                setRenameModalOpen(false);
                setItemToRename(null);
                refetch();
            } else {
                toaster.error(result.Message || 'Failed to rename item');
            }
        } catch (error: any) {
            const errorMessage = error?.data?.Message || error?.message || 'An error occurred while renaming item';
            toaster.error(errorMessage);
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={async () => {
                                            ...
                                            if (success) {
                                                toaster.success(`Path copied: ${itemPath}`);
                                            } else {
                                                toaster.error('Failed to copy path');
                                            }
                                        }}
