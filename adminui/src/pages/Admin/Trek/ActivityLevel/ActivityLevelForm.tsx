...
import { useGetActivityLevelByIdQuery, useSaveActivityLevelMutation } from "../../../../redux/trek/activityLevelAPI";
import { ActivityLevelSaveRequest } from "../../../../types/trekTypes";

const ActivityLevelForm = () => {
    const navigate = useNavigate();
    const location = useLocation();
    const [activityLevelId, setActivityLevelId] = useState(0);
    ...
    const {
        ...
        formState: { errors },
        register,
    } = useForm<ActivityLevelSaveRequest>({
        defaultValues: {
            activityLevelId: 0,
            name: "",
            description: "",
    ...
    const [saveActivityLevel, { isLoading: isSaving }] = useSaveActivityLevelMutation();
    const { data: detailData, isSuccess } = useGetActivityLevelByIdQuery(activityLevelId, { skip: !isEditMode || activityLevelId === 0 });

    useEffect(() => {
        if (location.pathname.includes("edit") && id) {
            setIsEditMode(true);
            setActivityLevelId(parseInt(id, 10));
        } else {
            setIsEditMode(false);
    ...
    const onSubmit = async (formData: ActivityLevelSaveRequest) => {
        try {
            const apiData = {
                ...formData,
                activityLevelId: isEditMode ? activityLevelId : 0,
            };
            ...
            if (response.Code == 200) {
                ...
                navigate("/admin/trek/activitylevel");
            } else {
                toaster.error("Failed to save activity level.");
            }
        } catch (error: any) {
        ...
        <>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div className="grid grid-cols-1 gap-4">
                    <ComponentCard title={`${isEditMode ? "Edit" : "New"} Activity Level`}>
                        <InputField
                            type="text"
                            id="name"
                            labelName="Name"
                ...
                <div className="mt-3 flex justify-end gap-3">
                    <button
                        type="button"
                        onClick={() => navigate("/admin/trek/activitylevel")}
