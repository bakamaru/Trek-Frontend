...
import { useDeleteTrekCategoryMutation, useGetAllTrekCategoryQuery } from "../../../../redux/trek/trekCategoryAPI";

interface IFilter {
    Name: any;
}

const FilterTrekCategory = ({
    control,
    handleSubmit,
    onFilterSubmit,
    handleFilterReset,
...
export default function TrekCategoryList() {
    ...
    const { data, isLoading, refetch } = useGetAllTrekCategoryQuery({ query: searchText, limit, offset, ...filterData });
    const [deleteTrekCategory] = useDeleteTrekCategoryMutation();
    ...
    const columns = [
        {
            ...
            label: "#No",
            render: (item: any) => {
                return item?.TrekCategoryId ? item?.TrekCategoryId : "N/A";
            },
        },
        {
            key: "name",
                    ...
                    <button
                        ...
                        onClick={() => {
                            if (confirm("Are you sure?")) {
                                handleDelete(row.TrekCategoryId);
                            }
                        }}
    ...
    const handleDelete = async (id: number) => {
        try {
            var response: any = await deleteTrekCategory(id).unwrap();
            if (response.Code == 200) {
                toaster.success("Trek Category deleted successfully!");
                refetch();
            } else {
                toaster.error("Failed to delete trek category!");
            }
        } catch (error) {
            toaster.error("An error occurred while deleting the trek category.");
        }
    };

    useEffect(() => {
        if (data != undefined && data.Code == 200) {
            setTrekCategories(data.Data);
    ...
    return (
        <>
            <div className="space-y-6">
                <ComponentCard title="Trek Category List">
                    <>
