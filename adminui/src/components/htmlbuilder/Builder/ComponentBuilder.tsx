...
const ComponentBuilder: React.FC<ComponentBuilderProps> = ({ onBack, initialData }) => {
    ...
    const handleSave = async () => {
        if (!name || !displayName) {
            toaster.error("Please enter a Name and Display Name");
            return;
        }
        ...
        try {
            ...
            if (uniqueCheckResult?.Data !== true) {
                toaster.error("Component name already exists. Please choose a different name.");
                return;
            }
        } catch (error: any) {
            toaster.error("Failed to validate component name. Please try again.");
            return;
        }
        ...
        try {
            ...
            if (response.Code === 200) {
                toaster.success("Component saved successfully!");
                onBack();
            } else {
                toaster.error(response.Message || "Failed to save component");
            }
        } catch (error: any) {
            toaster.error(error?.data?.Message || "An error occurred while saving");
        }
    };
        ...
        <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900 overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-brand-500 to-brand-600 px-6 py-4 flex items-center justify-between flex-shrink-0 shadow-lg">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-white">Component Builder</h1>
                        <p className="text-sm text-white/80">Design and configure reusable components</p>
                    </div>
                </div>
                <button
                    onClick={handleSave}
                ...
                >
                    ...
                    {initialData?.HtmlComponentId ? "Update Component" : "Save Component"}
                </button>
            </div>
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Component metadata and display settings</p>
                                    </div>
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Define your component structure using Handlebars syntax</p>
                                    </div>
                                    <button className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors">
