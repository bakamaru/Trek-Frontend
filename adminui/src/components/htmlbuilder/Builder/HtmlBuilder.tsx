...
const HtmlBuilder: React.FC = () => {
  ...
  const loadFromLocalStorageOrSample = () => {
    ...
    if (!html) {
      if (window.confirm("No saved layout found. Load sample?")) {
        html = SAMPLE_HTML;
      } else {
        return;
      }
  ...
  const loadHtmlString = (html: string) => {
    ...
    if (restoredBlocks.length === 0) {
      alert("No valid builder components found.");
      return;
    }
    setBlocks([]);
    setSelectedBlockId(null);
  ...
  return (
    <div className="flex h-[calc(100vh-160px)] w-full overflow-hidden bg-white font-sans border border-gray-200 rounded-xl shadow-lg">
      {/* LEFT SIDEBAR: Tools */}
              ...
              {templates.map((template) => {
                ...
                return (
                  <div
                    ...
                    className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 hover:border-blue-400 hover:bg-blue-50 transition cursor-grab active:cursor-grabbing bg-white shadow-sm select-none group"
                  >
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-400">Drag or Click</div>
                    </div>
                  </div>
                );
              })}
      ...
      <div className="flex-1 flex flex-col relative h-full min-w-0">
        ...
        <div className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-6 shadow-sm z-20">
          ...
          <div className="flex gap-2">
            <button onClick={saveToLocalStorage} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-green-200 text-green-700 rounded-md hover:bg-green-50 transition text-xs font-bold shadow-sm"><Save size={14} /> Save</button>
            <button onClick={loadFromLocalStorageOrSample} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-orange-200 text-orange-700 rounded-md hover:bg-orange-50 transition text-xs font-bold shadow-sm"><Upload size={14} /> Load</button>
            <div className="w-px h-6 bg-gray-300 mx-2"></div>
            <button onClick={() => setIsImportModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-50 border border-blue-200 text-blue-700 rounded-md hover:bg-blue-100 transition text-sm font-bold shadow-sm"><FileCode size={16} /> Import</button>
            <button onClick={copyHtml} className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-white rounded-md hover:bg-slate-900 transition text-sm font-bold shadow-sm active:scale-95 transform"><Code size={16} /> Copy HTML</button>
          </div>
        </div>
          ...
          <div ref={canvasRef} className={`${canvasWidthClass} bg-white shadow-2xl flex flex-col min-h-full ring-1 ring-black/5 rounded-lg`}>
            {blocks.length === 0 && (
              ...
              >
                ...
                <h3 className="text-lg font-medium text-gray-600 mb-1">Start Building</h3>
                <p className="text-sm opacity-60">Drag components from the sidebar to here</p>
              </div>
            )}
            ...
            {blocks.length > 0 && (
              ...
              >
                ...
                <p className="text-sm font-medium group-hover:text-blue-600">Drop component here to add at the end</p>
                <div className="text-[10px] mt-2 opacity-50 uppercase tracking-widest font-bold">End of Page</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col h-[80vh]">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2"><FileCode size={20} className="text-blue-600" />Import HTML</h2>
              <button onClick={() => setIsImportModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full text-gray-500"><X size={20} /></button>
            </div>
            <div className="flex-1 p-4 bg-gray-50 flex flex-col gap-2">
              <textarea className="flex-1 w-full p-4 border border-gray-300 rounded-lg font-mono text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none bg-white text-gray-900" value={importHtmlValue} onChange={(e) => setImportHtmlValue(e.target.value)} />
            </div>
            <div className="p-4 border-t border-gray-200 flex justify-end gap-3 bg-white rounded-b-xl">
              <button onClick={() => setIsImportModalOpen(false)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg transition">Cancel</button>
              <button onClick={handleImportSubmit} disabled={!importHtmlValue.trim()} className="px-6 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition disabled:opacity-50">Import & Render</button>
            </div>
          </div>
        </div>
      )}
