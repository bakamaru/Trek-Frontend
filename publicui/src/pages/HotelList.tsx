                ...
                <div className="grid lg:grid-cols-4 gap-8">
                    ...
                    <aside className="lg:col-span-1">
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md sticky top-28">
                            <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Filter Hotels</h2>
                            <FilterSection title="Star Rating">
                                <Checkbox label="5 Stars" count={4} />
                                <Checkbox label="4 Stars" count={0} />
                                <Checkbox label="3 Stars" count={0} />
                            </FilterSection>
                            <FilterSection title="Amenities">
                                <Checkbox label="Free WiFi" />
                                <Checkbox label="Swimming Pool" />
                                <Checkbox label="Spa" />
                                <Checkbox label="Pet Friendly" />
                            </FilterSection>
                            <FilterSection title="Price Range">
                                <input type="range" min="400" max="1000" className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700" />
                    ...
                    <main className="lg:col-span-3">
                         ...
                         <div className="flex flex-col sm:flex-row justify-between items-center mb-6 bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                            <p className="text-gray-600 dark:text-gray-300 mb-4 sm:mb-0">
                                Showing <span className="font-bold text-gray-900 dark:text-white">{sortedHotels.length}</span> hotels
                            </p>
                        ...
                        {loading ? (
                          ...
                          <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 gap-6" : "space-y-6"}>
                              {sortedHotels.map(hotel => (
                                  <div key={hotel.id} className={`bg-white dark:bg-gray-700 rounded-lg shadow-md overflow-hidden group ${viewMode === 'list' ? 'flex flex-col md:flex-row' : ''}`}>
                                      <div className={`relative ${viewMode === 'list' ? 'w-full md:w-64 flex-shrink-0' : ''}`}>
                                          <img src={hotel.image} alt={hotel.name} className={`w-full object-cover ${viewMode === 'list' ? 'h-48 md:h-full' : 'h-56'}`} />
                                      </div>
                                      <div className="p-4 flex-grow flex flex-col">
                                          <div className="flex justify-between items-start">
                                              <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 line-clamp-2">{hotel.name}</h3>
                                              {viewMode === 'grid' && (
                                          ...
                                          <div className="flex items-center mt-2 mb-4">
                                              ...
                                              <span className="text-gray-600 dark:text-gray-300 text-sm ml-2">({hotel.reviews} reviews)</span>
                                          </div>
                                          
                                          {viewMode === 'list' && (
                                               <div className="mt-auto flex justify-between items-end">
                                                    <div>
                                                         {/* Add description or amenities preview here if available in the data */}
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-2xl font-bold text-blue-700">${hotel.price}</p>
                                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">/ night</p>
                                                        <Link to={`/hotels/${hotel.id}`} className="inline-block bg-blue-700 text-white px-6 py-2 rounded-md font-semibold hover:bg-blue-800 transition-colors duration-300">
                                                            View Details
                                                        </Link>
                                                    </div>
                                               </div>
                                          ...
                                          {viewMode === 'grid' && (
                                              <div className="mt-auto">
                                                  <Link to={`/hotels/${hotel.id}`} className="w-full text-center block bg-blue-700 text-white px-4 py-2 rounded-md font-semibold hover:bg-blue-800 transition-colors duration-300">
                                                      View Details
                                                  </Link>
                                              </div>
                                          )}
