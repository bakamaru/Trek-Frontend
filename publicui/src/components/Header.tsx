...
import { useGetLoginStatusQuery } from '../redux/api/authAPI';
...
const Header: React.FC = () => {
  ...
  const { data: loginStatus, isLoading } = useGetLoginStatusQuery(undefined);
            ...
            {loginStatus?.isLoggedIn ? (
              <div className="relative" ref={userMenuRef}>
                <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="block focus:outline-none">
                  <img src={loginStatus.user?.profilePicture || "https://via.placeholder.com/150"} alt="User" className="w-10 h-10 rounded-full border-2 border-transparent hover:border-blue-700 transition" />
                </button>
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-700 rounded-md shadow-lg py-1 z-20 ring-1 ring-black ring-opacity-5">
                    <div className="px-4 py-2 border-b dark:border-gray-600">
                      ...
                      <p className="text-sm font-medium text-gray-900 truncate dark:text-white">{loginStatus.user?.name}</p>
                    </div>
              ...
              <Link to="/login" className="bg-blue-700 text-white px-4 py-2 rounded-md font-semibold hover:bg-blue-800 transition-colors duration-300">Login</Link>
            )}
      ...
      <div className={`lg:hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden bg-white dark:bg-gray-800`}>
        <nav className="flex flex-col p-4 space-y-2">
          ...
          {loginStatus?.isLoggedIn ? (
            ...
            <Link to="/login" className="w-full text-center bg-blue-700 text-white px-6 py-3 mt-2 rounded-md font-semibold hover:bg-blue-800 transition-colors duration-300">
              Login
            </Link>
          )}
        </nav>
