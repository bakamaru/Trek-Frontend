            ...
            {loginStatus?.isLoggedIn ? (
              <div className="relative" ref={userMenuRef}>
                <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="block focus:outline-none">
                  <img
                    src={loginStatus.user?.profilePicture || "https://via.placeholder.com/150"}
                    alt="User"
                    className="w-10 h-10 rounded-full border-2 border-transparent hover:border-blue-700 transition"
                  />
                </button>
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-700 rounded-md shadow-lg py-1 z-20 ring-1 ring-black ring-opacity-5">
                    ...
                    <Link
                      to="/user/dashboard/dashboard/profile"
                      className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-600"
                      onClick={() => setIsUserMenuOpen(false)}
                    >
                      My Profile
        ...
        <nav className="flex flex-col p-4 space-y-2">
          {/* {sortedMenuItems.map((link) => (
            ...
            <Link
              ...
              className="font-semibold text-gray-700 hover:text-blue-700 dark:text-gray-300 dark:hover:text-blue-400 p-2 rounded"
            >
              {link.label}
            </Link>
          ))} */}
